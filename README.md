...
Однако лучший результат из классических подходов показала BiLSTM, устранив главную проблему простых эмбеддингов: при обычном усреднении (Mean Pooling) вектор одного ругательства терялся в длинном тексте, а рекуррентная сеть смогла выловить этот сигнал в последовательности. 

Обычный BERT из-за своих размеров был вычислительно дорогим, но добавление **Fine-tuning через параметры LoRA (PEFT)** для модели `DistilBERT` позволило эффективно дообучить модель. Заморозка базовых весов позволила дообучить модель и показала высочайшее качество.
...
|-----------|-------------|
| ML/DL | PyTorch, Scikit-learn, Transformers (BERT), **PEFT (LoRA)**, Datasets |
...
├── lora-bert.ipynb            # Эксперименты: DistilBERT + PEFT (LoRA)
├── bert_tox_class.ipynb       # Эксперименты: Transformers (через эмбеддинги)
├── toxic_lora/                # Сохраненные веса LoRA адаптеров
