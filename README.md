...
│   ├── FastTextVectorizer.py  # Sklearn-совместимый FastText
│   ├── loader.py              # PyTorch Dataset и DataLoader
│   ├── train_LSTM.py          # Цикл обучения LSTM
│   ├── preprocess_data.py     # Очистка текста
│   ├── find_optimal_threshold.py  # Подбор порогов
│   ├── utils.py               # Метрики, визуализация
│   └── shap_show.py           # SHAP-анализ
├── main.ipynb                 # TF-IDF, FastText эксперименты
├── lstm.ipynb                 # BiLSTM эксперименты
├── bert_tox_class.ipynb       # BERT эксперименты
├── requirements.txt
└── README.md
```
